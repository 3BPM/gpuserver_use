...
def list_interfaces():
    ...
    try:
        ...
        for line in result.stdout.splitlines():
            ...
            if match:
                iface = match.group(1)
                if iface != 'lo' and re.search(r'eth|enp|ens|enx|wlan|tailscale', iface):
...
def arp_scan(network, iface=None):
    ...
    try:
        # 检查是否有arp-scan命令
        if subprocess.run(['which', 'arp-scan'], capture_output=True, text=True).returncode == 0:
            # 使用指定接口进行扫描
            cmd = ['arp-scan', '-l', '-g', '-x', '-r', '5', network]
            if iface:
            ...
            print(f"使用ping+arp扫描，请稍候 (1~254) ... ", end='', flush=True)
            ...
            if iface:
                ...
                try:
                    result = subprocess.run(['ip', 'neigh', 'show', 'dev', iface], 
                                          capture_output=True, text=True, check=True)
                    pattern = re.compile(r'^(\d+\.\d+\.\d+\.\d+)\s+lladdr\s+([0-9a-f:]+)')
                    for line in result.stdout.splitlines():
                        match = pattern.match(line)
                        if match:
                            devices.append((match.group(1), match.group(2)))
                except subprocess.CalledProcessError:
                    print(f"无法获取接口 {iface} 的ARP缓存")
            else:
                # 使用传统arp命令
                result = subprocess.run(['arp', '-n'], capture_output=True, text=True, check=True)
                pattern = re.compile(r'^(\d+\.\d+\.\d+\.\d+)\s+\(?.*\)?\s+([0-9a-f:]+)\s+')
                for line in result.stdout.splitlines():
                    match = pattern.match(line)
                    if match:
                        devices.append((match.group(1), match.group(2)))
...
def main(args=None):
    ...
    ipv4_devices = arp_scan(ipv4_network, iface)
    if ipv4_devices:
